...
def parquet_to_csv_weather(input_dir, output_dir):
    """
    Converte todos os arquivos .parquet dentro de input_dir em arquivos .csv no output_dir.
    Cada .parquet vira um .csv único.
    """
    output_tmp = "../data/output/weather/tmp_csv"
    spark = SparkSession.builder.getOrCreate()
    df = spark.read.parquet(input_dir)
    df.coalesce(1).write.csv(output_tmp, mode="overwrite", header=True)
    for tmp_file in os.listdir(output_tmp):
...
dim_local_weather = "../data/output/weather/dim_local"
dim_date_weather = "../data/output/weather/dim_date"
dim_time_weather = "../data/output/weather/dim_time"
dim_weather_code_weather = "../data/output/weather/dim_weather_code"
...
parquet_to_csv_weather(f"{dim_local_weather}.parquet", f"{dim_local_weather}.csv")
#parquet_to_csv_weather(f"{dim_date_weather}.parquet", f"{dim_date_weather}.csv")
#parquet_to_csv_weather(f"{dim_time_weather}.parquet", f"{dim_time_weather}.csv")
#parquet_to_csv_weather(f"{dim_weather_code_weather}.parquet", f"{dim_weather_code_weather}.csv")
#parquet_to_csv_weather(f"{fact_weather}.parquet", f"{fact_weather}.csv")
